...
def load_sample_data():
    ...
    try:
        ...
        current_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(os.path.dirname(current_dir))
        data_path = os.path.join(project_root, 'data', 'raw', 'sample_transactions.csv')
...
def main():
    ...
    # Header
    st.markdown('<h1 class="main-header">🔍 Payment Anomaly Detection System</h1>', unsafe_allow_html=True)
    
    # Load sample data
    df = load_sample_data()
    
    if df is None:
        st.error("Sample data not found. Please run the data generator first.")
